...
export function SubprojectHeader({ subproject, onSaved, onDeleted }) {
    ...
    async function remove() {
        if (!window.confirm(`Delete subproject "${subproject.name}" and all its tickets?`)) {
            return;
        }
    ...
    return (_jsx("div", { className: "h-full overflow-auto border-b border-border bg-card/40 px-6 py-4", children: !editing ? (_jsxs("div", { className: "flex items-start justify-between gap-6", children: [_jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("h2", { className: "truncate text-xl font-semibold tracking-tight", children: subproject.name }), _jsx(Badge, { variant: "outline", className: "uppercase tracking-wide", children: subproject.status })] }), _jsx("p", { className: "mt-1 max-w-3xl whitespace-pre-wrap text-sm text-muted-foreground", children: subproject.context_brief.trim() ||
                                "No context brief. The agent will have nothing to work from until this is filled in." })] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(Button, { variant: "ghost", size: "icon", onClick: () => {
                                reset();
                                setEditing(true);
                            }, "aria-label": "Edit subproject", children: _jsx(Pencil, { className: "h-4 w-4" }) }), _jsx(Button, { variant: "ghost", size: "icon", onClick: remove, "aria-label": "Delete subproject", className: "text-destructive-foreground/80 hover:bg-destructive/20", children: _jsx(Trash2, { className: "h-4 w-4" }) })] })] })) : (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Input, { value: name, onChange: (e) => setName(e.target.value), className: "h-9 max-w-md text-base font-semibold" }), _jsxs(Select, { value: status, onValueChange: (v) => setStatus(v), children: [_jsx(SelectTrigger, { className: "h-9 w-40", children: _jsx(SelectValue, {}) }), _jsx(SelectContent, { children: STATUSES.map((s) => (_jsx(SelectItem, { value: s, children: s }, s))) })] })] }), _jsx(Textarea, { value: brief, onChange: (e) => setBrief(e.target.value), rows: 3, placeholder: "Context brief used by the MCP agent for orientation." }), _jsxs("div", { className: "flex items-center justify-end gap-2", children: [_jsxs(Button, { variant: "ghost", size: "sm", onClick: () => {
                                reset();
                                setEditing(false);
